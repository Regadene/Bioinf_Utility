...
def test_filter_fastq(gc_bounds, length_bounds, quality_threshold, filtered_seqs_amount):
    filter_fastq(
    ...
    assert len(records) == filtered_seqs_amount, f"Expected {filtered_seqs_amount} sequences, got {len(records)}"
